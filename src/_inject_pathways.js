...
js = js.replace(
  "  recordToolUse('Contract & Offer Analyzer',null,winner",
  `  // Pathway
  ...
  var ocmActions2=[{text:'Run both contracts through the Contract Risk Assessment to catch hidden clauses.',when:'this week'},{text:'Model both salary trajectories in the Financial Planner for the 30-year picture.',when:'this week'},{text:'Negotiate the weaker offer\\u2019s terms up \\u2014 you have leverage with a competing offer.',when:'before signing'}];
  document.getElementById('ocm-result').innerHTML+=hwPathway(ocmPosition,ocmActions2,{id:'v11',icon:'\\ud83d\\udcc8',title:'Financial Planner',why:'See how each offer compounds over 30 years \\u2014 a $50K/year difference becomes millions.'});
  recordToolUse('Contract & Offer Analyzer',null,winner`
);
...
js = js.replace(
  "  recordToolUse('Financial Planner',null,'Y3 Wealth:",
  `  // Pathway
  ...
  h+='</div>';h+=hwPathway(fypPosition2,[{text:'Set up 401k + backdoor Roth this week. Every paycheck without them costs you.',when:'this week'},{text:(pslf==='yes'?'Certify PSLF employment \\u2014 do not refinance.':'Refinance your loans to lock a lower rate.')+' This is free money.',when:'this week'},{text:'Automate savings at '+yr1Save+'%'+(yr1Save<20?' (push to 20%)':'')+' before lifestyle creep takes over.',when:'this month'}],{id:'v11',icon:'\\ud83d\\udcb5',title:'Financial Planner',why:'Get a full financial health score \\u2014 disability, tax strategy, advisor quality, and spending optimization.'});
...
js = js.replace(
  "  recordToolUse('Financial Planner',score+'/'+maxScore",
  `  // Pathway
  ...
  var ilpNext=debt>100000?{id:'v11',icon:'\\ud83d\\udcc8',title:'Financial Planner',why:'Model your first 3 years month by month \\u2014 see exactly when you\\u2019ll be debt-free and how much you\\u2019ll build.'}:{id:'v11',icon:'\\ud83d\\udd2e',title:'Financial Planner',why:'Project your 30-year net worth under different career scenarios.'};
  document.getElementById('ilp-results').innerHTML+=hwPathway(ilpPosition2,[{text:flags.length?flags[0]+' \\u2014 fix this first.':'Keep doing what you\\u2019re doing.',when:'this week'},{text:flags.length>1?flags[1]:'Run the Financial Planner to see the 30-year impact.',when:'this month'},{text:'Review your contract \\u2014 your compensation structure directly drives everything above.',when:'this month'}],ilpNext);
  recordToolUse('Financial Planner',score+'/'+maxScore`
);
...
js = js.replace(
  ...
  if(ciResEl)ciResEl.innerHTML+=hwPathway(ciPosition2,ciActions2,{id:'v12',icon:'\\u2696\\ufe0f',title:'Contract & Offer Analyzer',why:'If you have multiple offers, compare total compensation side-by-side \\u2014 base salary alone is misleading.'});
  recordToolUse('Contract Review Tool',score+'/100',scoreLabel`
);
...
js = js.replace(
  ...
  `  }else{
    insEl.innerHTML=debtSection;
    insEl.innerHTML+=hwPathway('You have one scenario modeled. Add a second to see how different choices compare over 30 years.',[{text:'Add a second scenario (different salary, savings rate, or practice type).',when:'now'},{text:'Max your tax-advantaged accounts \\u2014 401k, backdoor Roth, HSA.',when:'this week'},{text:'Run the Financial Planner for your detailed first-3-years strategy.',when:'this week'}],{id:'v11',icon:'\\ud83d\\udcc8',title:'Financial Planner',why:'The 30-year view shows where you\\u2019re going. The 3-Year Planner shows you exactly how to start.'});
  }`
);

// Tool: v10 Career Transition - add pathway before recordToolUse
js = js.replace(
  "  recordToolUse('Specialty & Career Fit',readiness+'/4'",
  `  // Pathway
