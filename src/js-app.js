...
function renderLeverage(){
  ...
  if(!hasAccess&&!isCore){
    ...
    if(workflows)workflows.style.padding='0 24px';
    document.getElementById('lev-workflow-list').innerHTML='<div style="text-align:center;padding:60px 0"><div style="font-size:48px;margin-bottom:16px">\ud83d\udd12</div><div style="font-size:16px;font-weight:600;color:var(--text);margin-bottom:8px">Elite Strategy Members Only</div><p style="font-size:13px;color:var(--text3);line-height:1.6;margin-bottom:20px">Strategic Leverage tools are available exclusively to Elite Strategy subscribers.</p><button class="btn btn-a" onclick="navTo(\'scr-profile\');showUpgrade()" style="max-width:240px;margin:0 auto">View Elite Plan \u2192</button></div>';
    return;
  }
  if(isCore){
    if(hero)hero.style.display='none';
