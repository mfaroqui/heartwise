...
function getScoreActions(cp,scores){
  ...
  // ===== MEDICAL STUDENT =====
  if(stage==='student'){
    if(goal==='match'||goal==='specialty'||!goal){
  ...
  if(stage==='resident'){
    ...
    if(goal==='finance'){
      ...
      if(debt>200000) actions.push({action:'Evaluate PSLF eligibility — could save $200K+',gain:'Saves $200K+',priority:'high'});
  ...
  if(stage==='fellow'){
    ...
    if(goal==='finance'){
      ...
      if(debt>0) actions.push({action:'Finalize PSLF vs refinance decision before attending salary starts',gain:'Saves $100K+',priority:'high'});
  ...
  if(stage==='attending'){
    if(goal==='contract'||!goal){
      ...
      if(comp>0) actions.push({action:'Benchmark your compensation against MGMA data for '+(spec||'your specialty'),gain:'Know your worth',priority:'high'});
    ...
    if(goal==='finance'){
      var debt=parseInt(String(cp.debt||'0').replace(/[^0-9]/g,''))||0;
      var nw=parseInt(String(cp.networth||'0').replace(/[^0-9]/g,''))||0;
      ...
      if(debt>100000) actions.push({action:'Accelerate loan payoff — '+(debt>200000?'consider refinancing':'aggressive payoff plan'),gain:'Saves $'+Math.round(debt*0.15/1000)+'K+ in interest',priority:'high'});
    ...
    if(goal==='direction'){
      ...
      if(sat>0&&sat<6) actions.push({action:'Run the Career Pivot Decision Engine — your satisfaction is '+sat+'/10',gain:'Clarity',priority:'high'});
