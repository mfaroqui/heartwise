...
function renderLeverage(){
  ...
  if(!hasAccess){
    // Hide all elite-only sections
    if(hero)hero.style.display='none';
    if(filters)filters.style.display='none';
    if(quiz)quiz.style.display='none';
    if(progress)progress.style.display='none';
    if(workflows)workflows.style.padding='0';
    // Show gate message
    var gateHtml='<div style="text-align:center;padding:60px 24px">';
    gateHtml+='<div style="font-size:40px;margin-bottom:16px">\ud83d\udd12</div>';
    gateHtml+='<div style="font-family:var(--font-serif);font-size:20px;font-weight:600;color:var(--text);margin-bottom:12px">Strategic Leverage is an Elite Feature</div>';
    gateHtml+='<p style="font-size:14px;color:var(--text2);line-height:1.7;margin-bottom:24px;max-width:380px;margin-left:auto;margin-right:auto">This section includes 10 interactive strategy tools covering research, networking, career planning, and more. It\u2019s available exclusively with an Elite Strategy subscription.</p>';
    gateHtml+='<div style="display:flex;flex-direction:column;gap:10px;max-width:280px;margin:0 auto">';
    gateHtml+='<button class="btn btn-a" onclick="navTo(\'scr-profile\');showUpgrade()">View Elite Plan \u2192</button>';
    gateHtml+='<button style="font-size:12px;color:var(--text3);background:none;border:none;cursor:pointer;padding:8px" onclick="navTo(\'scr-vault\')">Browse Free Frameworks</button>';
    gateHtml+='</div></div>';
