...
function admRenderToolActivity(c){
  ...
  h+='<input type="text" id="adm-ta-search" placeholder="Search user name/email..." oninput="admFilterToolActivity()" style="flex:1;min-width:150px;font-size:12px;padding:8px 12px;border:1px solid var(--border);border-radius:8px;background:var(--bg3);color:var(--text)">';
...
function admFilterToolActivity(){
  ...
  var filtered=runs.filter(function(r){
    ...
    if(search&&r.user.toLowerCase().indexOf(search)===-1&&r.email.toLowerCase().indexOf(search)===-1)return false;
    return true;
  });
