...
function renderLeverage(){
  if(!U)return;
  // Gate: Elite (non-trial) or Admin gets full access; Core sees upgrade prompt
  ...
  if(isCore){
    // Core users see the page but with a gate overlay
    document.getElementById('lev-quiz-section').style.display='none';
    document.getElementById('lev-workflow-list').innerHTML='<div style="text-align:center;padding:48px 24px"><div style="font-size:52px;margin-bottom:16px">\u26a1</div><div style="font-family:var(--font-serif);font-size:22px;font-weight:600;color:var(--text);margin-bottom:10px">This Is Where Elite Trainees Separate Themselves</div><p style="font-size:14px;color:var(--text2);line-height:1.7;margin-bottom:8px;max-width:380px;margin-left:auto;margin-right:auto">The Strategic Leverage playbook contains 10 advanced workflows that top trainees use to outperform their peers in learning, research, networking, and career strategy.</p><p style="font-size:13px;color:var(--text3);line-height:1.6;margin-bottom:24px;max-width:380px;margin-left:auto;margin-right:auto">This feature is exclusive to Elite Strategy members and is not available during the trial period.</p><div style="display:flex;flex-direction:column;gap:10px;max-width:280px;margin:0 auto"><button class="btn btn-a" onclick="navTo(\'scr-profile\');showUpgrade()">Upgrade to Elite \u2192</button><button style="font-size:12px;color:var(--text3);background:none;border:none;cursor:pointer;padding:8px" onclick="navTo(\'scr-vault\')">Browse Free Frameworks Instead</button></div></div>';
    return;
  }
  // Ensure quiz section is visible for Elite users
